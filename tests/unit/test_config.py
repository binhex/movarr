...
class TestConfigMigration:
    ...
    def test_v1_config_is_migrated_to_v2(self, tmp_path: Path) -> None:
        ...
        cfg = load_config(str(cfg_file))
        assert cfg.general.config_version == "2.7.0"
...
class TestMigrationV25toV26:
    ...
    def test_existing_config_at_v26_is_migrated_to_v27(self, tmp_path: Path) -> None:
        """A config at v2.6.0 is migrated to v2.7.0 (good_ → allow_ rename)."""
        cfg_file = tmp_path / "config.yml"
        cfg_file.write_text(
    ...
    def test_existing_config_at_v27_needs_no_migration(self, tmp_path: Path) -> None:
        """A config already at v2.7.0 is not re-migrated."""
        cfg_file = tmp_path / "config.yml"
